...
impl BorrowCk<'_> {
    ...
    fn check_borrows_block(&mut self, b: &Block, active: &mut Vec<Borrow>) {
    ...
    fn check_borrows_stmt(&mut self, s: &Stmt, active: &mut Vec<Borrow>) {
        match s {
            Stmt::Let { pattern, value, .. } => {
    ...
    fn check_borrows_expr(&mut self, e: &Expr, active: &mut Vec<Borrow>) {
        match &e.kind {
            ExprKind::Unary {
    ...
    fn check_access(&mut self, place: &Place, exclusive: bool, pos: Pos, active: &[Borrow]) {
    ...
    fn access_place_or_recurse(&mut self, e: &Expr, exclusive: bool, active: &mut Vec<Borrow>) {
    ...
    fn check_place_indices(&mut self, e: &Expr, active: &mut Vec<Borrow>) {
        match &e.kind {
    ...
    fn method_mutates(&self, recv: &Expr, method: &str) -> bool {
    ...
    fn check_escape(&mut self, e: &Expr) {
        match &e.kind {
            ExprKind::Unary {
...
fn add_pattern_names(p: &Pattern, set: &mut HashSet<String>) {
    match p {
        Pattern::Binding(n) | Pattern::Typed { binding: n, .. } => {
            set.insert(n.clone());
        }
...
fn closure_free_vars(params: &[Param], body: &Expr) -> Vec<(String, NodeId)> {
