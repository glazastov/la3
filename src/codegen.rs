...
mod tests {
    ...
    // (they only diverge at narrow-width overflow). Asserting the
    // plainly-computed value is therefore asserting oracle parity for these
    // inputs.
    ...
    fn out_of_scope_function_is_skipped_not_miscompiled() {
        // A `&mut` reference parameter is still beyond scope (Phase 6.3), so
        // the function must be reported skipped rather than mis-translated.
        // (`str` is now supported as of 6.1, so it is no longer the example
        // here.)
        let ctx = Context::create();
    ...
    #[test]
    #[ignore = "needs str comparison lowering"]
    fn str_comparison_is_supported() {
        let ctx = Context::create();
        let (mir, oracle) = lower_to_mir("fn eq(a: str, b: str) -> bool { a == b }\nfn main() {}");
        let (_module, skipped) = build_program_module(&ctx, &mir, &oracle).expect("build module");
        assert!(
            !skipped.iter().any(|(sym, _)| sym == "eq"),
            "str comparison must compile once it is supported: {skipped:?}"
        );
    }

    #[test]
    #[ignore = "needs collection runtime lowering"]
    fn collection_parameter_is_supported() {
        let ctx = Context::create();
        let (mir, oracle) =
            lower_to_mir("fn size(xs: List<i64>) -> i64 { xs.len() as i64 }\nfn main() {}");
        let (_module, skipped) = build_program_module(&ctx, &mir, &oracle).expect("build module");
        assert!(
            !skipped.iter().any(|(sym, _)| sym == "size"),
            "collection-backed fn must compile once it is supported: {skipped:?}"
        );
    }

    // -- Phase 5.3: control flow from the MIR CFG (if/switch, loops,
    // break-value).
    ...
    // -- Phase 5.4: structs/tuples by value, enums as tagged unions, match
    // trees.
    ...
    fn enum_built_and_matched_within_one_function() {
        ...
        // yields an enum, copied by bytes), then match it — all in one
        // function.
        let ctx = Context::create();
        let ee = jit(
            &ctx,
